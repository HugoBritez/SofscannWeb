...
const handleLoteChange = (nuevoLote: string) => {
  ...
  const handleEditarArticulo = (articulo: Articulo) => {
    ...
    const articuloVencimiento = formatearVencimiento(articulo.al_vencimiento) || '';
  ...
  useEffect(() => {
    const fetchSucursalesYDepositos = async () => {
      try {
        ...
        const defaultDeposito = depositosData.body.find((deposito: any) => deposito.dep_inventario === 1)
        if (defaultDeposito) {
  ...
  const buscarArticuloPorCodigo = async (codigo: string) => {
    ...
    try {
      const codigoLimpio = codigo.startsWith('0') ? codigo.substring(1) : codigo
      ...
      if (!data || !Array.isArray(data.body)) {
        if (codigo !== codigoLimpio) {
          ...
          const responseOriginal = await fetch(`${API_URL}/articulos?${queryParamsOriginal}`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`,
    ...
    <div className="h-screen w-full flex flex-col overflow-hidden">
      ...
      <div className="bg-[#0455c1] rounded-b-3xl pb-4 z-10">
        <div className="flex  justify-between items-center px-4 pt-2 pb-4">
          ...
          <h2 className="text-white text-xs font-medium">{deposito?.dep_descripcion }</h2>
          </div>
          ...
          {articuloBusqueda && articulos.length > 0 ? (
            ...
            >
              {articulos.map((item) => (
                ...
                >
                  <p className="text-xs text-gray-500">Cod. Barras: {item.ar_codbarra}</p>
                  <p className="text-xs text-gray-500">Lote: {item.al_lote}</p>
                  <p className="text-xs text-gray-500">Vto.: {formatearVencimiento(item.al_vencimiento)}</p>
          ...
          <div className="bg-white rounded-lg w-11/12 max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold">Lote: {articuloSeleccionado?.al_lote}</h2>
                <button
                ...
                <div>
                  ...
                  >
                    {ubicaciones.map((ub: Ubicaciones) => (
                      <option key={ub.ub_codigo} value={ub.ub_codigo}>{ub.ub_descripcion}</option>
                    ))}
                  </select>
                </div>
                <div>
                  ...
                  >
                    {sububicaciones.map((sub) => (
                      <option key={sub.s_codigo} value={sub.s_codigo}>{sub.s_descripcion}</option>
                    ))}
                  </select>
                </div>
              </div>
